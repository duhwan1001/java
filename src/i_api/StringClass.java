...
public class StringClass {
	...
	public static void main(String[] args) {
		 ...
		 * - contains() : 문자열이 특정 문자열을 포함하고 있는가에 대한 여부를 반환한다.
		...
		System.out.println(equals); // 같으면 true 다르면 false
		
		String str1 = "문자열";
		String str2 = null;
		System.out.println(str1.equals(str2));
		// System.out.println(str2.equals(str1)); // null이 있을 수 있는 변수는 앞에 두지 않는다.
		...
		for(int i = 0; i < menu.length; i++){
			...
			int price = Integer.parseInt(menu[i].substring(menu[i].indexOf(" ") + 1, menu[i].indexOf("원"))); // Integer.parseInt 문자열을 숫자로 변환
		...
		//trim 
		//작성자의 실수를 보완해주기 위해 자주 사용함. ex) id&password
		str = " 문자열 ";
		String trim = str.trim();
		System.out.println("[" + str + "] -> [" + trim + "]");
		
		//사용자의 실수로 아이디나 비밀번호 앞뒤에 공백이 들어가는 것을 방지해 줄수 있다.
		String id = " myId ";
		...
		//valueOf
		int number = 10;
		str = number + ""; // int타입에 빈 문자열을 넣어 String 타입으로 형변환 시킴 (살짝편법)
